...
int mbedtls_dhm_test(void)
{
   ...
   // printf("Ciphertext: ");
    ...
    //int mode = MBEDTLS_ARIA_ENCRYPT; // 加密模式
    //mbedtls_aria_setkey_enc(&aria_ctx, digest, key_bitlen, mode);
    mbedtls_aria_setkey_enc(&aria_ctx, digest, key_bitlen);
    //printf("Ciphertext: ");
    unsigned char ciphertext1[16] = {0};
   // mbedtls_aria_crypt_ecb(&aria_ctx, MBEDTLS_ARIA_ENCRYPT, plaintext, ciphertext1);
